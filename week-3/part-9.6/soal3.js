...
function countProfit(shoppers) {
    ...
    for(let i = 0;i < listBarang.length ;i++){
        ...
        for(let j = 0;j < shoppers.length;j++){
            if(shoppers[j].product === listBarang[i][0] && shoppers[j].amount <= stokBarang){
                stokBarang -= shoppers[j].amount;
                listPembeli.push(shoppers[j].name)
            }
        }

        let profit = (listBarang[i][2] - stokBarang) * listBarang[i][1];

        hasil.push({
            product: listBarang[i][0],
            shoppers: listPembeli,
            leftOver : stokBarang,
            totalProfit: profit
        })
        console.log(hasil)
    ...
    //for(const items in shoppers){
      //  for(let i = o;i < shoppers.length;i++){
